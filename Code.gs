...
function isValidToken_(token) {
  ...
  // This runs on every single request (read or write), so it must not
  // scan the whole Sessions sheet — look up just this token's row and
  // read only its expiresAt cell.
  var sheet = getSheet_("Sessions");
  var rowIdx = findRowIndexById_(sheet, token);
  if (rowIdx === -1) return false;
  var lastCol = sheet.getLastColumn();
  var headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  var expCol = headers.indexOf("expiresAt");
  if (expCol === -1) return false;
  var expiresAt = sheet.getRange(rowIdx, expCol + 1, 1, 1).getValue();
...
function ensureHeaders_(sheet, keys) {
  ...
  var isNewSheet = headers.length === 0;
  if (isNewSheet) {
  ...
  if (missing.length) {
    ...
    // Plain-text formatting only needs to happen once per column, the
    // moment it's created — so Sheets never silently reinterprets a
    // date-like or number-like string as a Date/Number cell later.
    // This used to re-apply setNumberFormat("@") across up to 500 rows x
    // 24 cols on *every single write* regardless of whether the schema
    // changed, which was the single biggest source of save latency.
    // Now it only touches the specific new column(s), and only when a
    // new field is actually introduced.
    sheet.getRange(1, startCol, Math.max(sheet.getMaxRows(), 500), missing.length).setNumberFormat("@");
  }
  if (isNewSheet) {
    sheet.getRange(1, 1, Math.max(sheet.getMaxRows(), 500), 1).setNumberFormat("@");
  }
  return headers;
}
...
// Reads just the one row that matches `id`, instead of pulling every row
// in the sheet into memory (via sheetToObjects_) to find a single record.
// getOne_ is called multiple times per write in the stock/settings update
// chain (syncSaleStock_, applyManualStock_, updateStockEntry_, etc.), so
// this alone removes several full-sheet reads from every save.
function getOne_(collection, id) {
  ...
  var lastCol = sheet.getLastColumn();
  var headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  var rowArr = sheet.getRange(rowIdx, 1, 1, lastCol).getValues()[0];
  var obj = {};
  for (var c = 0; c < headers.length; c++) {
    if (!headers[c]) continue;
    obj[headers[c]] = coerce_(rowArr[c]);
