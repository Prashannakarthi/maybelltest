...
- **Attendance** — pick a date and every active employee shows up
  defaulting to **Present**; tap anyone who's absent to switch them, or use
  the trash icon on a row to remove that day's record entirely (with a
  confirmation first). Save everyone in one go. Month view shows a
  calendar for one employee, or a summary table for all.
...
- **Stock In/Out** — running qty/value totals. "Add Stock Entry" lets you
  ...
  Transfer)** — all three can be edited or deleted afterward, and doing so
  automatically reverses/reapplies their effect on the running totals.
  **Category B (Customer Sale/Return)** is generated automatically from
  what you enter in Daily Sales, so it can't be edited here directly —
  edit or delete the Daily Sales entry instead, which keeps both screens
  in sync (deleting a Daily Sales entry also removes its linked stock
  movement). This stays correct even if two people save at the same
  moment from different devices, since the backend serializes all writes.
...
- **If a change doesn't seem to show up** after redeploying both of the
  above, it's almost always the browser (or GitHub Pages' CDN) serving a
  cached copy of the old JS files. Hard-refresh the page (Ctrl/Cmd+Shift+R,
  or open the site in a private/incognito window) before concluding
  something is actually missing.

## A couple of things worth knowing

- **Every record type can be edited or deleted** (Employees, Attendance,
  Daily Sales, Stock entries, Expenses, Feedback, Targets) — deletions
  always ask for confirmation first and can't be undone afterward.
...
- **Sessions are per-tab by default** — closing the browser/tab clears
  your login, so reopening the app always asks for the password again.
  Check **"Remember me on this device"** at login if you'd rather stay
  signed in across restarts (up to 30 days); that's stored separately and
  doesn't affect the default behavior for anyone who doesn't check it.
  Signing out always fully reloads the app to guarantee a clean state.
- **All dates are normalized to plain YYYY-MM-DD** every time they're read
  from the Sheet, even if a date cell ever gets converted to a real Date
  type (which can happen if you type a date directly into the Sheet's UI
  instead of through the app) — so this self-corrects automatically and
  you won't see "Invalid Date" from older rows.
