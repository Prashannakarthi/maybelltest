...
loginForm.addEventListener("submit", async (e) => {
  ...
  try {
    await login(passwordInput.value, rememberCheckbox.checked);
    showApp();
  } catch (err) {
...
document.getElementById("logout-btn").addEventListener("click", async () => {
  await logout();
  // A hard reload guarantees no residual in-memory state (cached data,
  // an in-flight page render, etc.) can survive into the next session —
  // the login screen after this is a fully fresh app load.
...
function showLogin() {
  ...
  loginChecking.classList.add("hidden");
  loginForm.classList.remove("hidden");
  passwordInput.value = "";
  passwordInput.focus();
}

// The login form stays hidden and the app shell is never shown until this
// completes with an explicit, server-confirmed "yes" — there is no default
// or fallback path that shows protected content while this is pending.
