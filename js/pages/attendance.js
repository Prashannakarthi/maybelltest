import { getAll, batchUpsert, deleteRecord } from "../db.js";
import { el, toast, todayStr, confirmDialog } from "../utils.js";
...
export async function render(root) {
  ...
  const markCard = el("div", { class: "card card-pad" }, [
    el("div", { style: "display:flex;align-items:flex-end;gap:14px;flex-wrap:wrap" }, [
      ...
      el("div", { class: "help-text", style: "margin-bottom:9px" }, "Everyone defaults to Present — tap to switch anyone who's absent, or clear a row to remove that day's record entirely."),
    ]),
    rowsHost,
  ...
  const rowState = new Map(); // employeeId -> "Present" | "Absent" | null (null = not marked / cleared)
  let savedIds = new Set(); // employeeIds that already have a saved attendance row for this date
  ...
  async function loadRowsForDate() {
    ...
    savedIds = new Set(Object.keys(existingByEmp));
    ...
    activeEmployees.forEach((emp) => {
      ...
      const clearBtn = el("button", { type: "button", class: "icon-btn", title: "Remove this day's record" }, el("span", { "data-ic": "trash" }));
      const rowEl = el("div", { style: "display:flex;align-items:center;gap:10px;padding:9px 4px;border-bottom:1px solid var(--line)" }, [
        el("span", { style: "flex:1;font-size:14px" }, emp.name),
        presentBtn, absentBtn, clearBtn,
      ]);

      function setUI(status) {
        presentBtn.className = "btn btn-sm " + (status === "Present" ? "btn-gold" : "btn-ghost");
        absentBtn.className = "btn btn-sm " + (status === "Absent" ? "btn-gold" : "btn-ghost");
        rowEl.style.opacity = status ? "1" : "0.55";
      }
      presentBtn.addEventListener("click", () => { rowState.set(emp.id, "Present"); setUI("Present"); });
      absentBtn.addEventListener("click", () => { rowState.set(emp.id, "Absent"); setUI("Absent"); });
      clearBtn.addEventListener("click", async () => {
        if (savedIds.has(emp.id)) {
          const ok = await confirmDialog(`Remove ${emp.name}'s attendance record for this date? This can't be undone.`);
          if (!ok) return;
          try {
            await deleteRecord("attendance", `${emp.id}_${dateInput.value}`);
            savedIds.delete(emp.id);
            toast("Record removed", "success");
          } catch (err) {
            console.error(err);
            toast("Could not remove record", "error");
            return;
          }
        }
        rowState.set(emp.id, null);
        setUI(null);
      });

      rowsHost.appendChild(rowEl);
    });
    import("../icons.js").then((m) => m.hydrateIcons(rowsHost));
  }
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    try {
      const records = activeEmployees
        .filter((emp) => rowState.get(emp.id) != null)
        .map((emp) => ({
          id: `${emp.id}_${dateInput.value}`,
          merge: true,
          data: { employeeId: emp.id, employeeName: emp.name, date: dateInput.value, status: rowState.get(emp.id) },
        }));
      if (records.length) await batchUpsert("attendance", records);
