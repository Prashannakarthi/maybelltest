...
import { el, inr, monthStr, MONTHS_SHORT } from "../utils.js";
...
export async function render(root) {
  ...
  const [targetDoc, stockCounter, allSales] = await Promise.all([
    getOne("targets", nowMonth),
    getOne("settings", "stockCounter"),
    getAll("dailySales"),
  ]);

  const targetValue = targetDoc?.targetValue || 0;
  ...
  kpiGrid.appendChild(
    kpiCard("Target Value", inr(targetValue), targetValue ? "This month's target — tap to change" : "Not set — tap to set", true, () => { location.hash = "#/target"; })
  );
