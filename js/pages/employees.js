...
export async function render(root) {
  ...
  const tbody = el("tbody", { id: "emp-tbody" });
  ...
  function openEmployeeModal(emp) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (isEdit) {
          // Optimistic update: apply the edit to local state and repaint the
          // row immediately, before the network round-trip resolves. If the
          // save fails, the outer catch rolls this back and shows the error
          // (the modal stays open, same as before).
          const idx = allEmployees.findIndex((x) => x.id === emp.id);
          const previous = idx !== -1 ? allEmployees[idx] : null;
          if (idx !== -1) { allEmployees[idx] = { ...previous, ...data }; renderTable(); }
          try {
            await updateRecord("employees", emp.id, data);
          } catch (err) {
            if (idx !== -1) { allEmployees[idx] = previous; renderTable(); }
            throw err;
          }
        } else {
          const newId = await addRecord("employees", data);
          allEmployees.push({ id: newId, ...data });
