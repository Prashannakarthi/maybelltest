...
import { el, inr, toast, todayStr, monthStr, fmtDate, openModal, closeModal, confirmDialog, loadingRow, emptyRow, compareDates } from "../utils.js";
...
export async function render(root) {
  ...
  const expTbody = el("tbody", { id: "exp-tbody" });
  expTbody.innerHTML = loadingRow(5);
  ...
  function renderTable() {
    ...
    const rows = [...allExpenses].sort((a, b) => compareDates(b.date, a.date));
  ...
  function openExpenseModal(exp, prefillRecurring) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (isEdit) {
          // Optimistic update: patch local state and repaint immediately;
          // roll back only if the save itself actually fails.
          const idx = allExpenses.findIndex((x) => x.id === exp.id);
          const previous = idx !== -1 ? allExpenses[idx] : null;
          if (idx !== -1) { allExpenses[idx] = { ...previous, ...data }; renderAlerts(); renderTable(); }
          try {
            await updateRecord("expenses", exp.id, data);
          } catch (err) {
            if (idx !== -1) { allExpenses[idx] = previous; renderAlerts(); renderTable(); }
            throw err;
          }
        } else {
          const newId = await addRecord("expenses", data);
          if (data.month === viewMonth) allExpenses.push({ id: newId, ...data });
          renderAlerts();
          renderTable();
        }
        closeModal();
