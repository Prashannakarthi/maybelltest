    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (isEdit) {
          ...
          const idx = allFeedback.findIndex((x) => x.id === f.id);
          const previous = idx !== -1 ? allFeedback[idx] : null;
          if (idx !== -1) { allFeedback[idx] = { ...previous, ...data }; renderTable(); }
          try {
            await updateRecord("feedback", f.id, data);
          } catch (err) {
            if (idx !== -1) { allFeedback[idx] = previous; renderTable(); }
            throw err;
          }
        } else {
          const newId = await addRecord("feedback", data);
          allFeedback.push({ id: newId, ...data });
          renderTable();
        }
        closeModal();
