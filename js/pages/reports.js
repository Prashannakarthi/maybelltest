...
import { el, inr, fmtDate, todayStr, monthStr, downloadCSV, loadingRow, emptyRow, isDateInRange, compareDates } from "../utils.js";
...
async function buildReport(id, { from, to }) {
  switch (id) {
    case "daily-sales": {
      const sales = (await getAll("dailySales")).filter((s) => isDateInRange(s.date, from, to)).sort((a, b) => compareDates(a.date, b.date));
      return {
    ...
    case "month-sales": {
      ...
      const filtered = sales.filter((s) => isDateInRange(s.date, from, to));
    ...
    case "stock-report": {
      const entries = (await getAll("stockEntries")).filter((s) => isDateInRange(s.date, from, to)).sort((a, b) => compareDates(a.date, b.date));
    ...
    case "profit": {
      ...
      const s = sales.filter((x) => isDateInRange(x.date, from, to));
      const e = expenses.filter((x) => isDateInRange(x.date, from, to));
    ...
    case "expense-analysis": {
      const expenses = (await getAll("expenses")).filter((x) => isDateInRange(x.date, from, to));
      const byCat = {};
      expenses.forEach((x) => {
    ...
    case "feedback-report": {
      const feedback = (await getAll("feedback")).filter((x) => isDateInRange(x.date, from, to));
