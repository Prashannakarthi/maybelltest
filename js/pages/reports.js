...
const REPORTS = [
  ...
  { id: "month-sales", title: "B. Month-wise Sales Report", desc: "Sales totals grouped by month." },
  { id: "stock-report", title: "C. Stock In/Out Report", desc: "Movements by category, A/B/C." },
  ...
  { id: "month-end", title: "G. Month-End Summary", desc: "Sales, expenses & stock for one month." },
];
...
async function buildReport(id, { from, to }) {
  switch (id) {
    ...
    case "month-sales": {
      const sales = (await getAll("dailySales")).filter((s) => inRange(s.date, from, to));
      const byMonth = {};
      sales.forEach((s) => {
      ...
      return { columns: ["Month", "Sale Qty", "Sale Value", "Return Value", "Total Value"], rows };
    }
    case "stock-report": {
      const entries = (await getAll("stockEntries")).filter((s) => inRange(s.date, from, to)).sort((a, b) => (a.date < b.date ? -1 : 1));
      const byCat = { A: { In: 0, Out: 0 }, B: { In: 0, Out: 0 }, C: { In: 0, Out: 0 } };
      entries.forEach((e) => { if (byCat[e.category]) { byCat[e.category].In += e.stockIn || 0; byCat[e.category].Out += e.stockOut || 0; } });
      const summary = el("div", { class: "kpi-grid", style: "grid-template-columns:repeat(3,1fr);margin-bottom:14px" }, ["A", "B", "C"].map((c) =>
    ...
    case "month-end": {
      ...
      const [sales, expenses, stock] = await Promise.all([getAll("dailySales"), getAll("expenses"), getAll("stockEntries")]);
      ...
      const summary = el("div", { class: "kpi-grid", style: "margin-bottom:14px" }, [
        kpi("Total Sales", inr(totalSales)), kpi("Total Expenses", inr(totalExpenses)),
        kpi("Net (Sales − Expenses)", inr(totalSales - totalExpenses)),
        kpi("Stock In", String(netIn)), kpi("Stock Out", String(netOut)), kpi("Net Stock Change", String(netIn - netOut)),
      ]);
      return {
        columns: ["Metric", "Value"],
        rows: [
          { Metric: "Total Sales", Value: totalSales }, { Metric: "Total Expenses", Value: totalExpenses },
