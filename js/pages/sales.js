  ...
  function openSaleModal(sale) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        // We already know the full row we just wrote — merge it straight
        // into local state instead of re-fetching the whole collection.
        const idx = allSales.findIndex((s) => s.id === dateVal);
        if (idx !== -1) allSales[idx] = { ...allSales[idx], ...data, id: dateVal };
        else { allSales.push({ id: dateVal, ...data }); allSales.sort((a, b) => compareDates(b.date, a.date)); }
