...
import { syncSaleStockEntry } from "../stockSync.js";
...
export async function render(root) {
  ...
  const tableWrap = el("div", { class: "table-wrap" }, el("table", {}, [
    ...
    el("tbody", { id: "sales-tbody" }, loadingRow(9)),
  ]));
  ...
  async function handleDelete(sale) {
    const ok = await confirmDialog(`Delete the sales entry for ${fmtDate(sale.date)}? Its linked stock movement will remain — adjust Stock In/Out manually if needed.`);
    if (!ok) return;
  ...
  function openSaleModal(sale) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        allSales.sort((a, b) => (a.date < b.date ? 1 : -1));
