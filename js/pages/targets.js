...
import { el, inr, toast, confirmDialog, monthStr, fmtDate, loadingRow, emptyRow } from "../utils.js";
...
export async function render(root) {
  ...
  root.appendChild(el("div", { class: "table-wrap" }, el("table", {}, [
    el("thead", {}, el("tr", {}, ["Month", "Target Value", "", ""].map((h) => el("th", {}, h)))),
    tbody,
  ])));

  let targets = await getAll("targets", { orderBy: ["month", "desc"] });
  renderTable();
  prefillForm();
  ...
  function renderTable() {
    if (!targets.length) { tbody.innerHTML = emptyRow(4, "No targets set yet."); return; }
    tbody.innerHTML = "";
    targets.forEach((t) => {
      ...
      tbody.appendChild(el("tr", {}, [
        ...
        el("td", { class: "num" }, inr(t.targetValue)),
