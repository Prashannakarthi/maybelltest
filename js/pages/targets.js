...
export async function render(root) {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      await setRecord("targets", month, { month, targetValue: value });
      const idx = targets.findIndex((t) => t.id === month);
      if (idx !== -1) targets[idx] = { ...targets[idx], month, targetValue: value };
      else { targets.push({ id: month, month, targetValue: value }); targets.sort((a, b) => (a.month < b.month ? 1 : a.month > b.month ? -1 : 0)); }
      renderTable();
