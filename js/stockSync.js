...
// Edit/delete for manual (category A/C/O) stock entries only — category B
// rows are derived from Daily Sales and rejected server-side if targeted
// here, to keep the two screens from drifting apart.
export async function updateStockEntry(id, { date, category, stockIn, stockOut, value, storeName, remarks }) {
  const res = await apiCall("updateStockEntry", { id, date, category, stockIn, stockOut, value, storeName, remarks });
  invalidateCollectionCache("stockEntries");
  invalidateCollectionCache("settings");
  return { qty: res.qty, value: res.value };
}
...
export async function deleteStockEntry(id) {
  const res = await apiCall("deleteStockEntry", { id });
  invalidateCollectionCache("stockEntries");
  invalidateCollectionCache("settings");
  return { qty: res.qty, value: res.value };
}

// Called when a Daily Sales entry is deleted, to clean up the matching
// auto-generated category B stock row and reverse its effect on the
// running counter.
export async function deleteSaleStockEntry(dateStr) {
  const res = await apiCall("deleteSaleStock", { date: dateStr });
  invalidateCollectionCache("stockEntries");
  invalidateCollectionCache("settings");
  return { qty: res.qty, value: res.value };
}
